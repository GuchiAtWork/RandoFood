...
class ExampleUnitTest {
    ...
    class OpenHours(val open_now: Boolean = false)

    class Location(val lat: Float, val lng: Float)

    class Geometry(val location: Location)

    class Photo(val height: Int, val html_attributions: List<String>,
                val photo_reference: String = "", val width: Int)

    class Restaurant(val geometry: JsonObject? = null, val icon: String = "", val id: String = "",
                     val name: String = "", val opening_hours: JsonObject? = null,
                     val photos: JsonObject? = null, val place_id: String,
                     val reference: String = "", val types: List<String>, val vicinity: String = "")

    class GoogleResponse(val html_attributions: List<String>, val results: List<Restaurant>,
                         val status: String, val next_page_token: String = "")
    ...
    fun parsingGoogleJSON() {
        ...
        val klaxon = Klaxon()
        val parsed = klaxon.parseJsonObject(StringReader(jsonString))

        val restaurants: MutableList<HashMap<String, String?>> = ArrayList()
